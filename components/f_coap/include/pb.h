/* Minimal nanopb-compatible runtime for static protobuf messages */
#ifndef PB_H
#define PB_H
#include <stdint.h>
#include <stddef.h>
...
typedef struct pb_field_s { uint32_t tag; uint8_t type, htype, atype, _pad; size_t data_offset, data_size, size_offset; const void *ptr; } pb_field_t;
struct pb_ostream_s; struct pb_istream_s;
typedef struct pb_ostream_s { bool (*callback)(struct pb_ostream_s*,const pb_byte_t*,size_t); void *state; size_t max_size, bytes_written; bool has_error; } pb_ostream_t;
typedef struct pb_istream_s { bool (*callback)(struct pb_istream_s*,pb_byte_t*,size_t); void *state; size_t bytes_left; bool has_error; } pb_istream_t;
static inline pb_ostream_t pb_ostream_from_buffer(pb_byte_t *buf, size_t sz) { pb_ostream_t s={0}; s.max_size=sz; s.state=buf; return s; }
static inline pb_istream_t pb_istream_from_buffer(const pb_byte_t *buf, size_t sz) { pb_istream_t s={0}; s.bytes_left=sz; s.state=(void*)buf; return s; }
static inline bool pb_write(pb_ostream_t *s, const pb_byte_t *d, size_t n) { if(s->bytes_written+n>s->max_size){s->has_error=1;return false;} memcpy((pb_byte_t*)s->state+s->bytes_written,d,n); s->bytes_written+=n; return true; }
static inline bool pb_read(pb_istream_t *s, pb_byte_t *d, size_t n) { if(s->bytes_left<n){s->has_error=1;return false;} memcpy(d,s->state,n); s->state=(pb_byte_t*)s->state+n; s->bytes_left-=n; return true; }
static bool pb_encode_varint(pb_ostream_t *s, uint64_t v) { while(v>0x7F){pb_byte_t b=(v&0x7F)|0x80;if(!pb_write(s,&b,1))return false;v>>=7;} return pb_write(s,(pb_byte_t[]){v&0x7F},1); }
static bool pb_decode_varint(pb_istream_t *s, uint64_t *v) { *v=0; int sh=0; for(int i=0;i<10;i++){pb_byte_t b;if(!pb_read(s,&b,1))return false;*v|=((uint64_t)(b&0x7F))<<sh;sh+=7;if(!(b&0x80))return true;} return false; }
static uint8_t pb_wt(uint8_t lt) { switch(lt){case 0:case 1:return 0;case 3:return 1;case 4:case 5:return 2;case 2:return 5;default:return 0;} }
static bool pb_encode_tag(pb_ostream_t *s, uint8_t lt, uint32_t tag) { return pb_encode_varint(s,((uint64_t)tag<<3)|pb_wt(lt)); }
...
bool pb_encode(pb_ostream_t *s, const pb_field_t f[], const void *src) {
    for(int i=0;f[i].tag;i++) {
        const pb_field_t *fd=&f[i]; const pb_byte_t *p=((const pb_byte_t*)src)+fd->data_offset; size_t cnt=1;
...
bool pb_decode(pb_istream_t *s, const pb_field_t f[], void *dest) {
    while(s->bytes_left>0) {
        ...
        uint32_t tag=tw>>3; uint8_t wt=tw&7; int fi=-1;
        for(int i=0;f[i].tag;i++){if(f[i].tag==tag){fi=i;break;}}
        if(fi<0) {
        ...
        const pb_field_t *fd=&f[fi]; pb_byte_t *p=((pb_byte_t*)dest)+fd->data_offset;
