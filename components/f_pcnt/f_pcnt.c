...
esp_err_t f_pcnt_add_input(f_pcnt_handle_t handle, uint8_t gpio, uint8_t *unit_id_out)
{
    ...
    ESP_ERROR_CHECK(pcnt_channel_set_edge_action(chan, PCNT_CHANNEL_EDGE_ACTION_HOLD,
                                                 PCNT_CHANNEL_EDGE_ACTION_INCREASE));
    ESP_ERROR_CHECK(pcnt_channel_set_level_action(chan, PCNT_CHANNEL_LEVEL_ACTION_KEEP,
                             ...
                             uint16_t *rpm_out)
{
    ...
    *rpm_out =
        (uint16_t)(((uint64_t)pulse_count * 60000UL) / ((uint32_t)pulses_per_rev * interval_ms));
    return ESP_OK;
}
