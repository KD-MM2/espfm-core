...
def _u32(tag, v):
    return _wire(tag, 0, _v_encode(v))
...
def _f32(tag, v):
    return _wire(tag, 5, struct.pack("<f", v))


def _bool(tag, v):
    return _wire(tag, 0, b"\x01" if v else b"\x00")
...
def _decode(data):
    ...
    while pos < len(data):
        ...
        fn, wt = tw >> 3, tw & 7
        if wt == 0:
            v, pos = _v_decode(data, pos)
            result[fn] = ("v", v)
        elif wt == 5 and pos + 4 <= len(data):
            result[fn] = ("f", struct.unpack("<f", data[pos : pos + 4])[0])
            pos += 4
        elif wt == 2:
            ...
            try:
                s = sub.decode()
                result[fn] = ("s", s) if all(ord(c) < 128 for c in s) else ("b", sub)
            except:
                result[fn] = ("m", _decode(sub))
        else:
            break
    return result
...
def _fan_update(id, **kw):
    ...
    if "duty" in kw:
        m += _u32(3, kw["duty"])
    if "source_id" in kw:
        m += _u32(4, kw["source_id"])
    if "curve_id" in kw:
        m += _u32(5, kw["curve_id"])
    if "schedule_id" in kw:
        m += _u32(6, kw["schedule_id"])
    if "group_id" in kw:
        m += _u32(7, kw["group_id"])
    if "inverted" in kw:
        m += _bool(8, kw["inverted"])
    return m


def _src_create(typ, name, gpio=0):
    tm = {"ntc": 0, "ds18b20": 1, "manual": 2}
...
def _curve_create(name, pts):
    ...
    for tc, d in pts:
        m += _wire(
            2, 2, _v_encode(len(_f32(1, tc) + _u32(2, d))) + _f32(1, tc) + _u32(2, d)
        )
    return m
    ...
    if pts:
        for tc, d in pts:
            m += _wire(
                3,
                2,
                _v_encode(len(_f32(1, tc) + _u32(2, d))) + _f32(1, tc) + _u32(2, d),
            )
    return m
...
def _sched_update(id, **kw):
    ...
    if "duty" in kw:
        m += _u32(3, kw["duty"])
    if "start_min" in kw:
        m += _u32(4, kw["start_min"])
    if "end_min" in kw:
        m += _u32(5, kw["end_min"])
    if "enabled" in kw:
        m += _bool(6, kw["enabled"])
    return m
...
class ESPFM:
    ...
    def _r(self, method, path, payload=b""):
        ...
        print(
            f"  Status: {CODES.get(code, f'{code>>5}.{code&0x1F:02d}' if code else 'TIMEOUT')}"
        )
        if data:
            try:
                d = _decode(data)
    ...
    def fans(self, act, *a, **kw):
        if act == "list":
            ...
            self._r(GET, f"/fans/{a[0]}")
        elif act == "create":
            self._r(
                POST, "/fans", _fan_create(kw["pwm"], kw["name"], kw.get("tach", 0))
            )
        elif act == "update":
            self._r(PUT, f"/fans/{a[0]}", _fan_update(a[0], **kw))
        elif act == "delete":
            self._r(DELETE, f"/fans/{a[0]}")
    ...
    def sources(self, act, *a, **kw):
        if act == "list":
            ...
            self._r(
                POST, "/sources", _src_create(kw["type"], kw["name"], kw.get("gpio", 0))
            )
        elif act == "delete":
            self._r(DELETE, f"/sources/{a[0]}")
        elif act == "temp":
            tc = a[1] if len(a) > 1 else kw.get("temp_c", 0.0)
            self._r(POST, "/sources/temp", _temp(a[0] if a else kw.get("id", 0), tc))
    ...
    def curves(self, act, *a, **kw):
        if act == "list":
            ...
            self._r(GET, f"/curves/{a[0]}")
        elif act == "create":
            self._r(POST, "/curves", _curve_create(kw["name"], kw["points"]))
        elif act == "update":
            self._r(
                PUT,
                f"/curves/{a[0]}",
                _curve_update(a[0], kw.get("name"), kw.get("points")),
            )
        elif act == "delete":
            self._r(DELETE, f"/curves/{a[0]}")

    # Schedules
    def schedules(self, act, *a, **kw):
        if act == "list":
            self._r(GET, "/schedules")
        elif act == "create":
            self._r(
                POST,
                "/schedules",
            ...
            self._r(PUT, f"/schedules/{a[0]}", _sched_update(a[0], **kw))
        elif act == "delete":
            self._r(DELETE, f"/schedules/{a[0]}")

    # WiFi
    def wifi(self, act, **kw):
        if act == "scan":
            self._r(GET, "/wifi/scan")
        elif act == "status":
            self._r(GET, "/wifi/status")
        elif act == "connect":
            self._r(POST, "/wifi/connect", _wifi_conn(kw["ssid"], kw.get("pass", "")))

    # System
    def system(self, act):
        if act == "info":
            self._r(GET, "/system/info")
...
def main():
    ...
    f = sp.add_parser("fans")
    f.add_argument("action", choices=["list", "get", "create", "update", "delete"])
    f.add_argument("id", nargs="?", type=int)
    f.add_argument("--pwm", type=int)
    f.add_argument("--tach", type=int, default=0)
    f.add_argument("--name")
    f.add_argument("--mode", type=int, choices=[0, 1])
    f.add_argument("--duty", type=int)
    f.add_argument("--source", type=int, dest="source_id")
    f.add_argument("--curve", type=int, dest="curve_id")
    f.add_argument("--schedule", type=int, dest="schedule_id")
    f.add_argument("--group", type=int, dest="group_id")
    f.add_argument("--inverted", type=int, choices=[0, 1])

    s = sp.add_parser("sources")
    s.add_argument("action", choices=["list", "create", "delete", "temp"])
    s.add_argument("id", nargs="?", type=int)
    s.add_argument("--type")
    s.add_argument("--name")
    s.add_argument("--gpio", type=int, default=0)
    s.add_argument("--temp", type=float, dest="temp_c")

    c = sp.add_parser("curves")
    c.add_argument("action", choices=["list", "get", "create", "update", "delete"])
    c.add_argument("id", nargs="?", type=int)
    c.add_argument("--name")
    c.add_argument("--points")

    sc = sp.add_parser("schedules")
    sc.add_argument("action", choices=["list", "create", "update", "delete"])
    sc.add_argument("id", nargs="?", type=int)
    sc.add_argument("--fan", type=int, dest="fan_id")
    sc.add_argument("--duty", type=int)
    sc.add_argument("--start", type=int, dest="start_min")
    sc.add_argument("--end", type=int, dest="end_min")
    sc.add_argument("--enabled", type=int, choices=[0, 1])

    w = sp.add_parser("wifi")
    w.add_argument("action", choices=["scan", "status", "connect"])
    w.add_argument("--ssid")
    w.add_argument("--pass", dest="password", default="")

    sy = sp.add_parser("system")
    sy.add_argument("action", choices=["info"])
    ...
    try:
        if args.cmd == "fans":
            ...
            if args.inverted is not None:
                kw["inverted"] = bool(args.inverted)
            ...
            if args.name:
                kw["name"] = args.name
            if args.points:
                kw["points"] = _pts(args.points)
            e.curves(args.action, args.id, **kw)
        elif args.cmd == "schedules":
            kw = {}
            key_map = {"fan_id": "fan", "duty": "duty",
